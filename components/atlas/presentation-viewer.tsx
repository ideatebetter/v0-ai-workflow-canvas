      ...
      >
        ...
        {view === "masonry" && (
          <div style={{ columns: Math.min(images.length, 4), columnGap: "10px" }}>
            {images.map((img) => (
              <div key={img.id} className="break-inside-avoid mb-2.5 rounded-lg overflow-hidden" style={{ backgroundColor: "var(--app-card-elevated)" }}>
                {img.fileType === "video" ? (
        ...
        {view === "grid" && (
          ...
          >
            {images.map((img) => (
              <div key={img.id} className="relative rounded-lg overflow-hidden" style={{ backgroundColor: "var(--app-card-elevated)", aspectRatio: "4/3" }}>
                {img.fileType === "video" ? (
...
function BentoCellMedia({ node, focal }: { node: Node; focal: FocalPoint }) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <span style={{ fontSize: 24, color: "var(--app-text-muted)" }}>{fileData.fileExtension?.toUpperCase()}</span>
    </div>
  );
}
